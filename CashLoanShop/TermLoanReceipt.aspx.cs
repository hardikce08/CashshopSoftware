...
{
    public partial class TermLoanReceipt : System.Web.UI.Page
    {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["Id"]))
            {
                ...
                using (TermLoanService cc = new TermLoanService())
                {
                    ...
                    if (objcc != null)
                    {
                        ...
                        {
                            lblAdminFee.Text = "$" + Math.Round((objcc.LoanAmountApproved * (CompanyStores.AdminFeePercentage / 100)), 2).ToString();
                            lblInterestCharge.Text = "$" + Math.Round((objcc.LoanAmountApproved * (CompanyStores.TermInterestRate / 100)), 2).ToString();
